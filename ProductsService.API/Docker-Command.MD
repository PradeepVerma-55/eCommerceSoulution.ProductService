-- MYSQL Container from Docker Repository

docker run -it -e MYSQL_ROOT_PASSWORD=admin -p 3307:3306 --hostname=mysql-host-productsmicroservice --name mysql-products --network productsmicroservice-network -v C:\mysql-init\init.sql:/docker-entrypoint-initdb.d/init.sql mysql:latest
...
-- using the existing images
 docker run -p 8080:8080 --network=productsmicroservice-network -e MYSQL_HOST=mysql-host-products-microservice -e MYSQL_PASSWORD=admin 900325302/ecommerce-products-microservice:v1.0



 -- Database file mapping for initialization
